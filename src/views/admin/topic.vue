  ...
  <div style="position: absolute;left: 170px;top: 50px;">
    ...
    <el-link href="" v-for="(obj ) in topicCategorys" :type="getRandColor()" target="_blank">
      <el-popover placement="top-start" :title="obj.name" :width="200" trigger="hover"
        :content="`主题数量->${obj.subjectCount}个`">
        <template #reference>{{ obj.id + "." + obj.name }}</template>
      </el-popover>
    </el-link>
    <!-- 页面数据 -->
    <div>
      <el-button round @click="showDialog(true)" type="primary">添加话题</el-button>
    </div>
      ...
      <el-table :data="topics" style="width: 100%;">
        ...
        <el-table-column label="操作列表" width="280">
          <template #default="scope">
            <el-button round :icon="Edit"   @click="showDialog(false, scope.row)">编辑</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :title="wantAdd ? '话题添加' : '话题修改'" :width="500">
      <el-form ref="dialogFormRef" :model="topic" :rules="rules">
        <el-form-item label="话题类别2" prop="categoryId" :label-width="formLabelWidth">
          <el-select v-model="topic.categoryId" class="m-2" placeholder="Select" size="large">
            <el-option
                v-for="item in topicCategorys"
                :key="item.id"
                :label="item.name"
                :value="item.id"
            />
